...
import { usePeriodCompetencia } from "@/hooks/usePeriodCompetencia";

const paymentFormSchema = z.object({
    pagoEm: z.string().min(1, "Data de pagamento é obrigatória"),
    comprovante: z.custom<FileList | undefined>().optional(),
...
export default function FinancialPage() {
    ...
    const {
        ...
        onMonthChange,
        onYearChange,
    } = usePeriodCompetencia();

    const filteredTransactions = useMemo(
        () => period === "all" ? transactions : transactions.filter((transaction) => transaction.mesAno === period),
        [period, transactions]
    );
    ...
    const selectedPeriodLabel = period === "all"
        ? "Todos os períodos"
        ...
        <div className="space-y-6 animate-in fade-in duration-500">
                ...
                <CardContent className="p-4 md:p-5">
                    <div className="grid grid-cols-1 lg:grid-cols-3 gap-3 md:gap-4">
                        <div className="space-y-2">
                            <p className="text-xs font-medium text-slate-500 uppercase tracking-widest">Período</p>
                        ...
                        <div className="space-y-2 lg:col-span-2">
                            <p className="text-xs font-medium text-slate-500 uppercase tracking-widest">Navegação Rápida</p>
                            <div className="flex flex-wrap items-center gap-2 md:gap-3">
                                <MonthYearNavigator
                                    ...
                                    className="w-full"
                                    monthTriggerClassName="h-10 w-full sm:w-[170px]"
                                    monthContentClassName="rounded-lg"
                                    yearNavigatorClassName="w-full sm:w-auto gap-2"
                                    yearSelectTriggerClassName="h-10 w-[112px]"
                                    yearSelectContentClassName="rounded-lg"
                                />
                                <div
                                    key={`${period}-${filteredTransactions.length}`}
                                    className="period-transition-enter h-10 px-3 bg-slate-50 border border-slate-100 rounded-lg flex items-center text-sm text-slate-500 font-medium"
                                >
                            ...
                            {loading ? (
                                ...
                                filteredTransactions.map((t) => (
                                    <TableRow key={t.id} className="hover:bg-slate-50/50 transition-colors border-slate-100">
                                        ...
                                        <TableCell className="font-semibold font-outfit text-slate-700 text-base">
                                        ...
                                        <TableCell>
                                            <Badge
                                                ...
                                                className={t.status === 'Pago'
                                                    ? 'bg-primary/10 text-primary border-primary/20 font-semibold px-2.5 py-0.5 shadow-sm'
                                                    : 'bg-amber-100/80 text-amber-700 hover:bg-amber-200/80 border-amber-200/50 font-semibold px-2.5 py-0.5 shadow-sm'}
                                            >
                                            ...
                                            {(t.status === 'Em aberto' || t.status === 'Pago') ? (
                                                <div className="flex justify-end gap-2">
                                                    ...
                                                    <Button
                                                        ...
                                                        className={`rounded-lg shadow-md transition-all font-semibold gap-1.5 hover:scale-[1.02] ${t.status === 'Pago' ? 'border-primary text-primary hover:bg-primary/5' : 'bg-primary hover:bg-primary/90 border-none text-primary-foreground hover:shadow-lg'}`}
                                                    >
                ...
                <DialogContent className="sm:max-w-[450px] p-0 overflow-hidden border-none shadow-2xl rounded-xl">
                    ...
                    <div className="relative bg-primary px-6 py-5">
                        <div className="relative flex items-center gap-3">
                            ...
                            <div>
                                <DialogTitle className="text-lg font-semibold text-primary-foreground leading-none">Pagar Comissão</DialogTitle>
                                ...
                                {selectedTransaction && (
                                    <div className="mb-6 p-4 rounded-lg bg-slate-50 border border-slate-100">
                                        ...
                                        {selectedTransaction.status === 'Pago' ? (
                                                        ...
                                                        <FormControl>
                                                            <div className="relative">
                                                                ...
                                                                <Input
                                                                    ...
                                                                    className="pl-10 h-10 rounded-lg border-slate-200 bg-white font-semibold text-lg text-primary focus-visible:ring-primary focus-visible:border-primary transition-all shadow-sm"
                                                                />
                                                            </div>
                                                        </FormControl>
                                            ...
                                            <p className="text-3xl font-semibold text-primary font-outfit">{formatCurrency(selectedTransaction.valorTotal)}</p>
                                        )}
                                        ...
                                        {isReversingPending && (
                                            <div className="mt-4 p-3 rounded-lg bg-orange-100 border border-orange-200 flex items-start gap-3 animate-in slide-in-from-top-2 duration-300">
                                                ...
                                                <div>
                                                    <p className="font-medium text-orange-800 text-xs uppercase">Estorno em Andamento</p>
                                ...
                                {!isReversingPending ? (
                                    <div className="space-y-4 animate-in fade-in duration-300">
                                        <FormField
                                            ...
                                            render={({ field }) => (
                                                <FormItem className="space-y-1.5">
                                                    <FormLabel className="text-xs font-medium text-slate-600 uppercase tracking-wider">Data de Pagamento</FormLabel>
                                                    <FormControl>
                                                        <div className="relative">
                                                            <CalendarIcon className="absolute left-3 top-1/2 -translate-y-1/2 h-4 w-4 text-slate-400" />
                                                            <Input type="date" {...field} className="pl-10 h-10" />
                                                        </div>
                                                    </FormControl>
                                                    <FormMessage className="text-xs" />
                                                </FormItem>
                                        ...
                                        <FormField
                                            ...
                                            render={({ field }) => (
                                                <FormItem className="space-y-1.5">
                                                    <FormLabel className="text-xs font-medium text-slate-600 uppercase tracking-wider">Anexar Comprovante (Opcional)</FormLabel>
                                                    <FormControl>
                                                        <div className="relative">
                                                            ...
                                                            <Input
                                                                ...
                                                                className="pl-10 h-10 rounded-lg border-slate-200 bg-slate-50/50 focus-visible:ring-primary focus-visible:bg-white transition-colors file:mr-4 file:py-1 file:px-4 file:rounded-full file:border-0 file:text-xs file:font-medium file:bg-primary/10 file:text-primary hover:file:bg-primary/20"
                                                                name={field.name}
                                                                onBlur={field.onBlur}
                                                                ref={field.ref}
                                    ...
                                    <div className="h-40 flex flex-col items-center justify-center bg-orange-50/30 border-2 border-dashed border-orange-200 rounded-xl mb-6">
                                        <RotateCcw className="h-10 w-10 text-orange-300 mb-2 animate-pulse" />
                                        <p className="text-orange-600 font-medium text-sm">Pronto para Estornar</p>
                                ...
                                <DialogFooter className="pt-6 gap-2 sm:gap-0 flex-col sm:flex-row border-t border-slate-100 mt-2">
                                    <div className="flex gap-2">
                                        {selectedTransaction?.status === 'Pago' && (
                                            <Button
                                                ...
                                                className={`rounded-lg border-none font-semibold gap-2 transition-all mr-auto ${isReversingPending
                                    ...
                                    <div className="flex gap-2">
                                        <Button
                                            ...
                                            className="rounded-lg border-slate-200 text-slate-600 hover:bg-slate-50 hover:text-slate-800 font-semibold"
                                        >
                                            Cancelar
                                        </Button>
                                        <Button
                                            ...
                                            className={`rounded-lg shadow-lg transition-all hover:scale-[1.02] border-none font-semibold gap-2 ${isReversingPending
            ...
            <Dialog open={isDeleteConfirmOpen} onOpenChange={setIsDeleteConfirmOpen}>
                <DialogContent className="sm:max-w-[400px] p-0 overflow-hidden border-none shadow-2xl rounded-xl">
                    ...
                    <div className="p-6">
                        <div className="flex items-start gap-4 p-4 rounded-lg bg-rose-50 border border-rose-100 mb-6">
                            <AlertCircle className="h-5 w-5 text-rose-600 mt-0.5" />
                            <div className="text-sm">
                                <p className="font-semibold text-rose-900 mb-1 leading-tight">Impacto Sistêmico</p>
                ...
                <DialogContent className="sm:max-w-[450px] p-0 overflow-hidden border-none shadow-2xl rounded-xl">
                    ...
                    <div className="p-6">
                        <div className="flex items-start gap-4 p-4 rounded-lg bg-amber-50 border border-amber-100 mb-6">
                            ...
                            <div>
                                <p className="font-semibold text-amber-900 mb-1">Não é possível processar</p>
